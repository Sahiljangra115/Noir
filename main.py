...
class RobotBrain:
    ...
    def run(self) -> None:
        # ── Open camera ───────────────────────────────────────────────────
        dev = int(self.device) if self.device.lstrip("-").isdigit() else self.device
        cap = cv2.VideoCapture(dev)
        if not cap.isOpened():
            sys.exit(
                f"[ERROR] Cannot open camera '{self.device}'.\n"
                "  For phone camera:  scrcpy --video-source=camera --v4l2-sink=/dev/video3 --no-audio --max-fps 30 --max-size=1080\n"
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--device", default="/dev/video3",
                   help="Camera device path or index (default: /dev/video3)")
    ...
    p.add_argument("--laptop", action="store_true",
                   help="Use laptop webcam (device 1) instead of /dev/video3")
...
def main() -> None:
    args = _parse_args()
    brain = RobotBrain(
        device     = "1" if args.laptop else args.device,
        esp_host   = args.host,
        esp_port   = args.port,
